...
h.mark = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.895 0.2 0.0475 .04], 'String', 'Add point','FontWeight','Bold', 'Callback', ...
    {@mark_button_fcn,gcf});

h.threshfind_button = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.9475 0.2 0.0475 .04], 'String', 'Find Edge','FontWeight','Bold', 'Callback', ...
    {@threshfind_button_fcn,gcf});


ty = 0.00;
h.afWarp = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.895 0.165-ty 0.055 .03], 'String', 'Affine Warp','FontWeight','Bold', 'Callback', ...
    {@afwarp_button_fcn,gcf});

h.bsWarp = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.895 0.1325-ty 0.055 .03], 'String', 'b-spline Warp','FontWeight','Bold', 'Callback', ...
    {@bswarp_button_fcn,gcf});

h.intensityBsWarp = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.895 0.1-ty 0.055 .03], 'String', 'itkx bs Warp','FontWeight','Bold', ...
    'TooltipString', ['runs an intensity based b-spline warp with the initial affine'], ...
    'Callback', {@intensity_bswarp_button_fcn,gcf}); %#ok<NBRAK> 

h.SkipAf_checkbox = uicontrol('Style', 'checkbox', 'Value', 0,'Units','normalized' ...
    ,'BackgroundColor',[1 1 1],'Position',[0.905 0.075-ty 0.055 0.015],'HorizontalAlignment','left', ...
    'BackgroundColor', [1 1 1], 'String', 'Skip affine', 'FontSize', 10);  

h.SavePts = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.955 0.13-ty 0.04 .03], 'String', 'Save points','FontWeight','Bold', 'Callback', ...
    {@save_button_fcn,gcf});

h.LoadPts = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.955 0.165-ty 0.04 .03], 'String', 'Load points','FontWeight','Bold', 'Callback', ...
    {@load_button_fcn,gcf});

h.DeletePts = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.955 0.095-ty 0.04 .03], 'String', 'Delete points','FontWeight','Bold', 'Callback', ...
    {@delete_button_fcn,gcf});

h.LoadSWC = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.955 0.065-ty 0.04 .025], 'String', 'Load SWC','FontWeight','Bold', 'Callback', ...
    {@loadSWC_button_fcn,gcf});
...
h.ExportTif = uicontrol('Style', 'pushbutton',  'Units', 'normalized', 'Position', ...
    [.895 0.03-ty 0.1 .03], 'String', 'Export Tif','FontWeight','Bold', 'Callback', ...
    {@export_button_fcn,gcf});
...
max_channel=9;
h.panChannel = uipanel('Title','Loaded image channel','FontSize',12, 'Units', 'Normalized', ...
    'BackgroundColor',pcol, 'Position',[0.895 0.275 0.1 0.045]);
h.numChannelsSlider = uicontrol('Parent', h.panChannel, 'Style', 'Slider', 'Units', 'normalized', 'Position', ...
    [0.15 0.15 0.84 0.6], 'Min', 1, 'Max', max_channel, 'Value', 1, 'SliderStep', 1./(max_channel)+[0 0], ...
    'Callback', {@updateChannelSliderLoad,gcf}, 'BackgroundColor', [0.75 0.75 0.75]);
h.numChannelText = uicontrol('parent', h.panChannel, 'Style', 'text', 'String', ['1/' num2str(max_channel)],...
    'Units','normalized' ,'BackgroundColor',pcol,'Position',[0.015 0.15 0.06 0.7],'HorizontalAlignment','Left', ...
    'FontSize', 10);
guidata(h.fig, h);
...
function loadSWC_button_fcn(~, ~, fig)
h = guidata(fig);

[fn,path] = uigetfile('*.swc','Select SWC file', pwd);
if length(fn)<2
    return;
end
[pts, offset] = loadSWC(fullfile(path, fn));

answer = inputdlg({'X scale'; 'Y Scale'; 'Z scale'}, 'Upsample factor' , 1, {'1'; '1'; '1'});

xsc = str2double(answer{1});
ysc = str2double(answer{2});
zsc = str2double(answer{3});

x = pts(:,3)+offset(1);
y = pts(:,4)+offset(2);
z = pts(:,5)+offset(3);

answer = inputdlg({'Render levels'; 'Downsampple tree';}, 'Parameters' , 1, {'6'; '2';});
levels = str2double(answer{1});
ds = str2double(answer{2});

inds = 1:ds:size(pts,1);
scale = 2^levels;
...
h.tree.mov = [x(inds) y(inds) z(inds)];

a = [x(inds) y(inds) z(inds) ones(numel(inds), 1)];
if h.haveaf
    a = a*h.af;
end
if h.havebs
    a(1:3) = bspline_trans_points_double(h.O_trans_inv, h.Spacing_inv, a([2 1 3]));
    a = a(:, [2 1 3]);
end
...
guidata(fig, h);
disp('Done Loading SWC');


function [pts, offset] = loadSWC(swcfn)
...
fgetl(fid);
offset =  fgetl(fid);
offset =  sscanf(offset, '%*s %*s %f %f %f');
cnt = 1;
pts = zeros(7, Npts);
while (~feof(fid))
    str = fgetl(fid);
    pts(:,cnt) = sscanf(str, '%d %f %f %f %f %f %d');
    cnt = cnt+1;
end
fclose(fid);
...
function intensity_bswarp_button_fcn(~, ~, fig)
itxBspline2D(fig);
...
function threshfind_button_fcn(~,~,fig)
h = guidata(fig);
p1 = h.point(1).Position;
p2 = h.point(2).Position;
...
pts = [linspace(x(1), x(2), Npts)' linspace(y(1), y(2), Npts)'];

proff = interp2(double(h.fixed(:,:,z1)), pts(:,1), pts(:,2), 'cubic'); 
profc = interp2(double(h.current(:,:,z2)), pts(:,1), pts(:,2), 'cubic'); 
xx = (1:size(pts, 1))';

midf = findThresh(xx, proff);
midc = findThresh(xx, profc);

p1 = [pts(round(midf), 1) pts(round(midf), 2)];
p2 = [pts(round(midc), 1) pts(round(midc), 2)];
...
function export_button_fcn(~, ~, fig)

h = guidata(fig);
def_fn = ['Registered_' date '.tif'];

[fn,path] = uiputfile('*.tif','Select file name', fullfile(pwd, def_fn));
if ~fn
    return
end
disp('Writing file...');
% writeTifFast(fullfile(path, fn), h.current, 16);
writeTiff16(h.current, fullfile(path, fn));
